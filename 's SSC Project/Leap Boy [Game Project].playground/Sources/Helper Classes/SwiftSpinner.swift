...
public class SwiftSpinner: UIView {
    ...
    public class var sharedInstance: SwiftSpinner {
        struct Singleton {
            public static let instance = SwiftSpinner(frame: CGRect.zero)
        }
...
//        let randomRotation = Double(Float(arc4random()) /  Float(UInt32.max)) * M_PI_4 + M_PI_4
        let randomRotation = Double(Float(arc4random()) /  Float(UInt32.max)) * (Double.pi / 4.0) + (Double.pi / 4.0)
...
//            self.currentInnerRotation += CGFloat(M_PI_4)
            self.currentInnerRotation += (CGFloat.pi / 4.0)
